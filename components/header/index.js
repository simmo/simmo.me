...
export default class Header extends Component {
    _scrollToContent() {
        smoothScrollTo(document.body, this.wrapper.clientHeight, 1000)
    }
        ...
        return (
            <header className="header" ref={(el) => this.wrapper = el} id="top">
                <div className="header__wrap" ref={(el) => this.content = el}>
                    ...
                    <button className="header__cta" onClick={this._scrollToContent.bind(this)}>Next</button>
                </div>
            </header>
        )
    }
